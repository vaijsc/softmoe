...
class SoftMoELayerWrapper(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        norm_matrix = (x - mean)/ std
        # Permute for compatibility with matmul
        similarity_matrix = torch.matmul(norm_matrix, norm_matrix.transpose(1, 2))  # [batch_size, seq_length, seq_length]
        # import ipdb; ipdb.set_trace()
        similarity_matrix = torch.tril(similarity_matrix)
        y = torch.matmul(similarity_matrix, y)  # [batch_size, seq_length, dim]        
        return y
